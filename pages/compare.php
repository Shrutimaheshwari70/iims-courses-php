...
.cmp-grid.g1 { grid-template-columns: minmax(0, 380px); }
.cmp-grid.g2 { grid-template-columns: repeat(2, minmax(0,1fr)); }
.cmp-grid.g3 { grid-template-columns: repeat(3, minmax(0,1fr)); }
...
.cmp-card:hover .cmp-img img { transform: scale(1.06); }
.cmp-img-shade {
  position: absolute;
  inset: 0;
...
.cmp-loc i { color: var(--co); font-size: .82rem; }
...
.cmp-pill i { font-size: .8rem; color: var(--co); }
...
.btn-details:hover { opacity: .85; transform: translateY(-1px); color: #fff; }
...
.btn-rm:hover { background: #fee2e2; color: #b91c1c; }
...
.cmp-tbl tbody tr:nth-child(even) { background: rgba(244,246,251,.65); }
.cmp-tbl tbody tr:hover td         { background: rgba(45,61,107,.05); }
.cmp-tbl td {
  padding: 1rem 1.2rem;
...
.btn-browse:hover { opacity: .86; transform: translateY(-2px); color: #fff; }
.cmp-steps {
  display: grid;
...
.cmp-step-t { font-weight: 600; font-size: .9rem; color: var(--cfg); margin-bottom: .3rem; }
.cmp-step-d { font-size: .78rem; color: var(--cmuted); line-height: 1.5; }

/* ──────────────────────────────────────────
   RESPONSIVE
   ────────────────────────────────────────── */
@media (max-width: 900px) {
  .cmp-grid.g3 { grid-template-columns: repeat(2, minmax(0,1fr)); }
}
@media (max-width: 640px) {
  .cmp-grid.g3,
  .cmp-grid.g2 { grid-template-columns: minmax(0,1fr); }
  .cmp-hdr     { flex-direction: column; align-items: flex-start; }
  .cmp-lbl     { min-width: 7.5rem; }
  .cmp-tbl td  { padding: .75rem .9rem; font-size: .8rem; }
  .cmp-tbl thead th { padding: .85rem .9rem; font-size: .75rem; }
  .cmp-wrap    { padding: 4rem 0 3.5rem; }
}
</style>
...
<section class="cmp-empty">
  <div class="cmp-empty-inner">
    ...
    <p class="cmp-empty-desc">Add up to 3 IIMs and instantly compare fees, placements, rankings, and more — all in one clean view.</p>
        ...
        <div class="cmp-card">
          ...
          <div class="cmp-body">
            ...
            <div class="cmp-pills">
              ...
              <span class="cmp-pill"><i class="bi bi-graph-up-arrow"></i><?= htmlspecialchars($c['placement']) ?>L Avg</span>
            ...
            <div class="cmp-actions">
              ...
              <a href="compare.php?remove=<?= urlencode($c['slug']) ?>" class="btn-rm"
                 title="Remove" aria-label="Remove <?= htmlspecialchars($c['name']) ?>">
