...
function render_stars(float $rating): string {
    ...
    for ($i = 0; $i < $full; $i++)  $html .= "<span class=\"star filled\">$star</span>";
    if ($half)                       $html .= "<span class=\"star half\">$star</span>";
    for ($i = 0; $i < $empty; $i++) $html .= "<span class=\"star empty\">$star</span>";
    return $html;
}
...
function compare_add(string $slug): bool {
    ...
    if (compare_has($slug)) return true;
    if (count($_SESSION['compare']) >= 3) return false;
