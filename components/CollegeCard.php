 ...
 *     include 'components/CollegeCard.php';
 *   }
 * Expects variables: $college (array), $index (int)
 */
...
<div class="college-card fade-up" style="animation-delay:<?= $index * 0.06 ?>s">
  ...
  <div class="college-card-img">
    ...
    <div class="college-card-actions">
      <form method="post" action="actions/toggle-wish.php" style="display:contents">
        <input type="hidden" name="slug" value="<?= $college['slug'] ?>">
        <input type="hidden" name="redirect" value="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>">
        <button type="submit" class="card-icon-btn <?= $in_wish ? 'active' : '' ?>" title="<?= $in_wish ? 'Remove from wishlist' : 'Add to wishlist' ?>">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="<?= $in_wish ? 'currentColor' : 'none' ?>" stroke="currentColor" stroke-width="2">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
          </svg>
        </button>
      </form>
      <form method="post" action="actions/toggle-compare.php" style="display:contents">
        <input type="hidden" name="slug" value="<?= $college['slug'] ?>">
        <input type="hidden" name="redirect" value="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>">
        <button type="submit" class="card-icon-btn <?= $in_compare ? 'active' : '' ?>" title="<?= $in_compare ? 'Remove from compare' : 'Add to compare' ?>">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  <div class="college-card-body">
    ...
    <div class="college-card-footer">
      ...
      <a href="pages/college-details.php?slug=<?= $college['slug'] ?>" class="btn btn-soft btn-sm">
        View Details
      </a>
    </div>
  </div><!-- /college-card-body -->
