...
// ── Compare clear_all AJAX handler ──
if (isset($_GET['cmp_action']) && $_GET['cmp_action'] === 'clear_all') {
    $_SESSION['compare'] = [];
    header('Content-Type: application/json');
    echo json_encode(['count' => 0, 'action' => 'clear_all']);
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .mobile-menu .logo-sub { color: white !important; }
        ...
        .navbar-logo-icon img { width: 40px; height: 40px; }
        ...
        #megaCourses .mega-content { max-height: 420px; }
        #megaCourses .mega-content::-webkit-scrollbar { width: 6px; }
        #megaCourses .mega-content::-webkit-scrollbar-thumb { background: rgba(0,0,0,0.2); border-radius: 10px; }
        #megaCourses .mega-sidebar { max-height: 420px; overflow-y: auto; }
...
@keyframes cmpPop {
    ...
    100% {
        ...
        .dropdown-root { position: static !important; }
        ...
        .mega-sidebar { width: 180px; flex-shrink: 0; }
        .sidebar-label { font-size: 0.7rem; color: var(--muted-foreground, #64748b); }
        ...
        .content-header > div:first-child { flex: 1; min-width: 0; }
        .content-header h4 { font-size: 1rem; font-weight: 600; margin-bottom: 2px; }
        .content-header .count { font-size: 0.7rem; color: var(--muted-foreground, #64748b); }

        .mega-sidebar { border-right: 2px solid grey; padding: 10px; }
        ...
        @media (min-width: 1024px) {
            ...
            .navbar-main.scrolled .logo-sub { color: #0f172a !important; opacity: 1 !important; }
        }

        .navbar-logo-text .logo-sub { color: #0d1117; }
        ...
        /* ── Badge — hidden when 0, visible when >0 ── */
        .badge {
            position: absolute;
            top: -4px; right: -4px;
        ...
        /* Icon-btn active glow for compare */
        .icon-btn.cmp-active {
            border-color: #3b82f6;
            color: #3b82f6;
        }

        /* Icon-btn active glow for wishlist */
        ...
        .mobile-nav a,
        .mobile-menu-title,
        .close-menu-btn { color: #0f172a; }

        body.dark .mobile-nav a,
        body.dark .mobile-menu-title,
        body.dark .close-menu-btn { color: #ffffff; }
        ...
        .mobile-logo-icon img { width: 40px; height: 40px; }
        ...
        .modal-footer-text { text-align: center; font-size: 0.7rem; margin-top: 1rem; }
        ...
        #compareBar.bar-visible { transform: translateY(0); }
        ...
        .cmpbar-left { display: flex; align-items: center; gap: 1rem; flex: 1; min-width: 0; }
        ...
        .cmpbar-label svg { width: 14px; height: 14px; stroke: rgba(255,255,255,0.5); }

        .cmpbar-slots { display: flex; gap: 8px; align-items: center; flex-wrap: nowrap; overflow: hidden; }
        ...
        .cmpbar-right { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
...
<body>
    <header class="navbar-main" id="navbarMain" data-home="<?= $is_home ? 'true' : 'false' ?>">
            ...
            <div class="navbar-inner">
                ...
                <nav class="desktop-nav">
                    <div class="dropdown-root position-relative" id="dropdownCollegesRoot">
                        ...
                        <div class="mega-dropdown bg-white rounded shadow border py-3 px-2 overflow-hidden z-0" id="megaColleges">
                            <div class="mega-dropdown-inner d-flex gap-3">
                                <div class="mega-sidebar pr-4">
                                    <div class="sidebar-label text-uppercase mb-2 fw-semibold">Categories</div>
                                    <div class="category-list d-flex flex-column gap-1" id="collegeCategoriesList"></div>
                                </div>
                                <div class="mega-content">
                                    <div class="content-header justify-content-between gap-5 w-100 align-items-start mb-1" id="collegesContentHeader"></div>
                    ...
                    <div class="dropdown-root position-relative" id="dropdownCoursesRoot">
                        ...
                        <div class="mega-dropdown bg-white rounded shadow border py-3 px-2 overflow-hidden" id="megaCourses">
                            <div class="mega-dropdown-inner d-flex gap-3">
                                <div class="mega-sidebar pr-4">
                                    <div class="sidebar-label text-uppercase mb-2 fw-semibold">Course Types</div>
                                    <div class="category-list d-flex flex-column gap-1" id="courseCategoriesList"></div>
                                </div>
                                <div class="mega-content">
                                    <div class="content-header justify-content-between gap-5 w-100 align-items-start mb-1" id="coursesContentHeader"></div>
                ...
                <div class="navbar-actions">
                    <!-- Compare icon — id="navCmpBtn" for JS targeting -->
                    <a href="<?= $asset_base ?>pages/compare.php" class="icon-btn" id="navCmpBtn" aria-label="Compare">
                        ...
                        <span class="badge" id="cmpBadge"><?= $compareCount ?></span>
                    </a>

                    <!-- Wishlist icon — id="navWishBtn" for JS targeting -->
                    <a href="<?= $asset_base ?>pages/wishlist.php" class="icon-btn" id="navWishBtn" aria-label="Wishlist">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
                        </svg>
                        <span class="badge" id="wishBadge"><?= $wish_count ?></span>
                    </a>
        ...
        <div class="mobile-menu" id="mobileMenu">
            ...
            <nav class="mobile-nav d-flex flex-column px-2">
                ...
                <a class="p-2 border-bottom text-decoration-none py-3" href="<?= $asset_base ?>pages/colleges.php">IIMs Colleges</a>
                <a class="p-2 border-bottom text-decoration-none py-3" href="<?= $asset_base ?>pages/courses.php">Courses</a>
                <a class="p-2 border-bottom text-decoration-none py-3" href="<?= $asset_base ?>pages/about.php">About</a>
                <a class="p-2 border-bottom text-decoration-none py-3" href="<?= $asset_base ?>pages/blogs.php">Blogs</a>
                <a class="p-2 border-bottom text-decoration-none py-3" href="<?= $asset_base ?>pages/careers.php">Careers</a>
                <a class="p-2 border-bottom text-decoration-none py-3" href="<?= $asset_base ?>pages/compare.php">Compare</a>
                <a class="p-2 border-bottom text-decoration-none py-3" href="<?= $asset_base ?>pages/wishlist.php">Wishlist</a>
                <a class="p-2 border-bottom text-decoration-none py-3" href="<?= $asset_base ?>pages/contact.php">Contact</a>
        ...
        <div class="cmpbar-inner">
            <div class="cmpbar-left">
                <span class="cmpbar-label">
                    <svg viewBox="0 0 24 24" fill="none"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/></svg>
                    Compare
                </span>
    ...
    <script>
         ...
         * Shows/hides badge, animates pop when count increases.
         */
        ...
        /** Call on page load to set initial badge state */
        ...
        window.toggleWishlist = function(slug, btn) {
            ...
            fetch(url, { headers: { 'Accept': 'application/json' } })
                ...
                .then(function(data) {
                    // ✅ KEY FIX: update navbar badge instantly from server count
                    window.updateWishBadge(data.count);

                    // Also update navWishBtn icon style
                    var navBtn = document.getElementById('navWishBtn');
                    if (navBtn) {
                        navBtn.classList.toggle('wish-active', data.count > 0);
                    }
        ...
        /* ================================================================
           COMPARE — store + bar + badge (all instant, no refresh)
           ================================================================ */

        var _cmpStore = []; // { slug, name, image, location }
        ...
        window.renderCompareBar = function() {
            ...
            for (var i = _cmpStore.length; i < 3; i++) {
                html += '<a href="' + assetBase + 'pages/colleges.php" class="cmpbar-slot-empty">+ Add IIM</a>';
            }
...
window.renderCompareBar();
        ...
        /** Highlight/unhighlight the navbar compare icon */
        function _syncNavCmpIcon(count) {
            var navBtn = document.getElementById('navCmpBtn');
            if (navBtn) navBtn.classList.toggle('cmp-active', count > 0);
        }
document.addEventListener('DOMContentLoaded', function() {

    // Bootstrap compare store from active buttons
    ...
    // Wishlist
    initBadge('wishBadge', <?= $wish_count ?>);

    // Compare
    initBadge('cmpBadge', <?= $compareCount ?>);

    /* ─────────────────────────────
       NAV ICON STATES
    ───────────────────────────── */

    // Wishlist nav icon active
    var navWish = document.getElementById('navWishBtn');
    if (navWish) {
        navWish.classList.toggle('wish-active', <?= $wish_count ?> > 0);
    }

    // Compare nav icon active
    _syncNavCmpIcon(<?= $compareCount ?>);

    /* ─────────────────────────────
       FORCE REALTIME COUNTS
    ───────────────────────────── */

    // Force wishlist badge sync
    window.updateWishBadge(<?= $wish_count ?>);

    // Force compare badge sync
    window.updateCmpBadge(<?= $compareCount ?>);
    ...
    /* ─────────────────────────────
       SYNC ACTIVE COMPARE BUTTONS
    ───────────────────────────── */

    document.querySelectorAll('.cmp-toggle').forEach(function(btn) {
        var slug = btn.dataset.slug;

        var exists = _cmpStore.find(function(i) {
            return i.slug === slug;
        });
        ...
        /* ── Remove single slot from compare bar ── */
        document.addEventListener('click', function(e) {
            ...
            if (cardBtn) {
                window.toggleCompare(slug, cardBtn);
            } else {
                // Button not on this page — update store + session directly
                _cmpStore = _cmpStore.filter(function(i) { return i.slug !== slug; });
                window.updateCmpBadge(_cmpStore.length);
                _syncNavCmpIcon(_cmpStore.length);
                window.renderCompareBar();
                fetch(window.location.pathname + '?cmp_action=remove&cmp_slug=' + encodeURIComponent(slug),
                    { headers: { 'Accept': 'application/json' } }).catch(function(){});
            }
        });

        /* ── Global event delegation — .wl-toggle and .cmp-toggle ── */
        document.addEventListener('click', function(e) {
            // Wishlist
